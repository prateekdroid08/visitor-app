...
/*setting adapter*/
    ...
    public void onCompanyItemSelected(ResponseCompanies responseCompanies, int position) {
        Constants.DEFAULT_COMPANY_NAME = String.valueOf(responseCompanies.getCompanyLists().get(position).getCompanyName());
        Constants.DEFAULT_COMPANY_ID = String.valueOf(responseCompanies.getCompanyLists().get(position).getCompanyID());
        EventBus.getDefault().post(new MessageEvent("companyselected"));
